...
def create_attendance_excel(df, target_weeks, student_count):
    ...
    # 💡 핵심: wrap_text=False로 설정하여 자동 줄 바꿈 방지 (한 줄로만 표시)
    ...
    block_height = student_count + 7 
    student_row_height = max(15, min(28, int(650 / student_count)))
    ...
    for w_idx, current_week_start in enumerate(target_weeks):
        ...
        for d_idx, current_date in enumerate(dates_in_week):
            ...
            for p in range(1, 8):
                main_subj, sub_subj, cancel_reason = "", "", cancel_day
                is_class = False
                ...
                day_data.append({'p': p, 'main': main_subj, 'sub': sub_subj, 'cancel': cancel_reason, 'is_class': is_class})
        ...
        for ws in sheets.values():
            ...
            ws.merge_cells(start_row=start_row+1, start_column=1, end_row=start_row+3, end_column=1)
            ...
            ws.row_dimensions[start_row+3].height = 25
            
            for i in range(student_count):
                student_cell = ws.cell(row=start_row+4+i, column=1, value=f"학생{i+1}")
                student_cell.alignment = shrink_align; student_cell.border = thin_border
                ws.row_dimensions[start_row+4+i].height = student_row_height 
            ...
            for sheet_name, ws in sheets.items():
                ...
                for p_info in day_info['periods']:
                    ...
                    if p_info['cancel']:
                        ...
                        disp_list.append(p_info['sub'])
                
                start_p = 0
                while start_p < 7:
                    curr_subj = disp_list[start_p]
                    end_p = start_p
                    while end_p + 1 < 7 and disp_list[end_p + 1] == curr_subj:
                        end_p += 1
                    ...
                    for p_idx in range(start_p, end_p + 1):
                        ...
                        if not curr_subj or curr_subj.startswith("["):
                            ...
                            for r in range(start_row+4, start_row+4+student_count):
        ...
        for sheet_name in ["기초간호", "보건간호", "공중간호"]:
            ...
            for i, (s_name, c_name) in enumerate(zip(stats, cumuls)):
                ...
                ws.cell(row=start_row+3, column=stat_col+i).fill = stat_fill
                
                c2 = ws.cell(row=start_row+2, column=stat_col+5+i, value=c_name)
                c2.alignment = shrink_align; c2.font = stat_head_font
                ws.cell(row=start_row+3, column=stat_col+5+i).fill = cumul_fill
        ...
        for ws in sheets.values():
            ...
            for row in range(start_row+1, start_row+4+student_count):
...
st.title("📊 스마트 간호학원 출석부 (자동 줄바꿈 해제)")
